...
public interface ProductRepository extends ReactiveCrudRepository<Product, Long>, ProductRepositoryInternal {
    ...
    @Query("SELECT * FROM product entity WHERE entity.wish_list_id = :id")
    Flux<Product> findByWishList(Long id);

    @Query("SELECT * FROM product entity WHERE entity.wish_list_id IS NULL")
    Flux<Product> findAllWhereWishListIsNull();
