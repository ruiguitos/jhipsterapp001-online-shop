<template>
  <div>
    <h2 id="page-heading" data-cy="ProductHeading">
      <span v-text="$t('jhipsterapp001App.product.home.title')" id="product-heading">Products</span>
      <div class="d-flex justify-content-end">
        <button class="btn btn-info mr-2" v-on:click="handleSyncList" :disabled="isFetching">
          <font-awesome-icon icon="sync" :spin="isFetching"></font-awesome-icon>
          <span v-text="$t('jhipsterapp001App.product.home.refreshListLabel')">Refresh List</span>
        </button>
        <router-link :to="{ name: 'ProductCreate' }" custom v-slot="{ navigate }">
          ...
          >
            ...
            <span v-text="$t('jhipsterapp001App.product.home.createLabel')"> Create a new Product </span>
          </button>
        </router-link>
      </div>
    </h2>
    ...
    <div class="alert alert-warning" v-if="!isFetching && products && products.length === 0">
      <span v-text="$t('jhipsterapp001App.product.home.notFound')">No products found</span>
    </div>
        ...
        <thead>
          <tr>
            ...
            <th scope="row" v-on:click="changeOrder('title')">
              <span v-text="$t('jhipsterapp001App.product.title')">Title</span>
              <jhi-sort-indicator :current-order="propOrder" :reverse="reverse" :field-name="'title'"></jhi-sort-indicator>
            </th>
            <th scope="row" v-on:click="changeOrder('keywords')">
              <span v-text="$t('jhipsterapp001App.product.keywords')">Keywords</span>
              <jhi-sort-indicator :current-order="propOrder" :reverse="reverse" :field-name="'keywords'"></jhi-sort-indicator>
            </th>
            <th scope="row" v-on:click="changeOrder('description')">
              <span v-text="$t('jhipsterapp001App.product.description')">Description</span>
              <jhi-sort-indicator :current-order="propOrder" :reverse="reverse" :field-name="'description'"></jhi-sort-indicator>
            </th>
            <th scope="row" v-on:click="changeOrder('rating')">
              <span v-text="$t('jhipsterapp001App.product.rating')">Rating</span>
            ...
            <th scope="row" v-on:click="changeOrder('dateAdded')">
              <span v-text="$t('jhipsterapp001App.product.dateAdded')">Date Added</span>
            ...
            <th scope="row" v-on:click="changeOrder('dateModified')">
              <span v-text="$t('jhipsterapp001App.product.dateModified')">Date Modified</span>
              <jhi-sort-indicator :current-order="propOrder" :reverse="reverse" :field-name="'dateModified'"></jhi-sort-indicator>
            </th>
            <th scope="row" v-on:click="changeOrder('wishList.id')">
              <span v-text="$t('jhipsterapp001App.product.wishList')">Wish List</span>
              <jhi-sort-indicator :current-order="propOrder" :reverse="reverse" :field-name="'wishList.id'"></jhi-sort-indicator>
            </th>
            <th scope="row"></th>
          </tr>
        </thead>
          ...
          <tr v-for="product in products" :key="product.id" data-cy="entityTable">
            ...
            <td>
              <div v-if="product.wishList">
                <router-link :to="{ name: 'WishListView', params: { wishListId: product.wishList.id } }">{{
                  product.wishList.id
                }}</router-link>
              </div>
            </td>
    ...
    <b-modal ref="removeEntity" id="removeEntity">
      <span slot="modal-title"
        ><span id="jhipsterapp001App.product.delete.question" data-cy="productDeleteDialogHeading" v-text="$t('entity.delete.title')"
          >Confirm delete operation</span
        ></span
      >
      <div class="modal-body">
        <p id="jhi-delete-product-heading" v-text="$t('jhipsterapp001App.product.delete.question', { id: removeId })">
